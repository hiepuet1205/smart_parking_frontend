        ...
        <div class="col-3" v-for="slot in slotStore.getSlots" :key="slot.id">
          <q-card v-ripple class="my-box cursor-pointer q-hoverable">
            ...
            <q-card-section class="flex justify-between">
              ...
              <q-btn no-caps class="btn1" tag="router-link" :to="{ name: 'UpdateCoUserSlotPage', params: { slotId: slot.id } }">
...
import { getSlotOfCoUser } from 'src/api/slot.api';
...
onMounted(async () => {
  const data = await getSlotOfCoUser();
...
const handleSearch = async () => {
  const data = await getSlotOfCoUser(keyword.value);
